...
use SoftSteps\KintoneAPI\Client;
use SoftSteps\KintoneAPI\HttpClient\APIAuth;
use SoftSteps\KintoneAPI\HttpClient\HttpClientException;
//use SoftSteps\KintoneAPI\HttpClient\OAuth;
use SoftSteps\KintoneAPI\HttpClient\Options;
use SoftSteps\KintoneAPI\HttpClient\Request;
use SoftSteps\KintoneAPI\HttpClient\Response;
/**
...
class HttpClient
{
    ...
    {
        return str_starts_with($this->host, 'https://');
    }
     ...
     * @return string
     */
    protected function buildApiUrl($host)
    {
        $api = $this->options->apiPrefix();

        return \rtrim($host, '/') . $api . $this->options->getVersion() . '/';
    }
...
/*        } elseif( $this->options->isOAuthOnly() ) {
            $oAuth = new OAuth(
                $url,
                $this->api_token,
